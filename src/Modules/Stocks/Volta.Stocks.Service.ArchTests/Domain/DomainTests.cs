...
namespace Volta.Stocks.Service.ArchTests.Domain
{
    public class DomainTests : TestBase
    {
        ...
        //Important if using Entity Framework
        [Fact]
        ...
        [Fact]
        public void Domain_Object_Should_Not_Have_Public_Setters_On_Properties()
        {
            var domainObjectTypes = Types.InAssembly(DomainAssembly)
                .That()
                        .Inherit(typeof(Entity))
            ...
            foreach (var domainObjectType in domainObjectTypes)
            {
                var properties = domainObjectType.GetProperties(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
                foreach (var property in properties)
                {
